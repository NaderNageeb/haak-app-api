...
$service_detials = filterRequest("service_detials");
...
// insert into db
$sql_check = "SELECT * FROM `services` where
 `service_name` = '$service_name' and `service_del` = 0";
...
if(mysqli_num_rows($query_check)){
    ...
    $sqli_insert = "insert into services(`service_name`,`service_details`,`service_image`) 
    values ('$service_name','$service_detials','$new_file_name')";
...
if(mysqli_num_rows($query_check)){
   ...
   echo json_encode(array("status"=>"Exist"));
    ...
    $sqli_insert = "insert into services(`service_name`,`service_details`) 
    values ('$service_name','$service_detials')";
   if($query_insert = mysqli_query($conn,$sqli_insert))
   {
